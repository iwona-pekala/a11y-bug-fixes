...
2. Is it Android TalkBack or iOS VoiceOver?
   ...
   - If **Yes**, this is iOS VoiceOver **AND** there is a role, which is not read **AND** There is no name – Add a name, VoiceOver doesn't read dialog role, when a name is missing. Retest. Go to the [Role](#role) section if the issue persists.
...
3. Does the <dialog> tag have `role` attribute?
   ...
   - If **No** – go the the [Name](#name) section
