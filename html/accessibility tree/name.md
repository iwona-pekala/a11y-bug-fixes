...
The accessibility name is the information about the user interface element, allowing to is distinguish several elements with the same [role](role.md). It is used by assistive technologies, like screen readers. Name [often matches the visible label](label-name.md), but not always.
...
### Sample calculation order for an element with the textbox role (like `<input type="text">`)
...
### Sample calculation order for an element with the button role (like `<button>`)
1. Use the `aria-labelledby` attribute if present and valid, else
2. Use the `aria-label` attribute if present and valid, else
3. Use the `<label>` tag if present and valid, else
