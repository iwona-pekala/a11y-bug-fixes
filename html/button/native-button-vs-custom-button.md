...
The native `<button>` has very little requirements. It may ocasionally need some tweaks. In most cases it's enough to leave it as it is and make as little modifications as possible. Let's imagine an empty page with a button:
```
...
- ✔ Button can be activated with Enter ***and*** Space keys - a `<button>` can be activated with Enter ***and*** Space keys by default. They both perform will call `onclick()`.
...
As we can see almost evey checklist element is met by default without any extra effort.
...
There were no extra step for the `<button>`. The onclick event handles also Space and Enter keys (onclick is called for Enter keydown and Space keyup). 
...
- `onkeydown` for Space. Space down scrolls a page by default, so this needs to be prevented to match a `<button>` behavior and to avoid really anoyying behavior
