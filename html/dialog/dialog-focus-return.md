...
## Case `role="dialog"`
Once the dialog is closed, return focus using the `focus()` method.
...
## Case `<dialog>` element
The modal was displayed by using the showModal() method. Now it can be closed using the `close()` method. Once it's done, focus will automatically return to the element that opened the dialog, as long as this element still exists and it's still focusable. There are no extra steps.
